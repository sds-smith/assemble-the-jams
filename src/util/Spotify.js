...
const Spotify = {
    ...
    getAuthCode() {
        if (authCode) {
            ...
            window.location.replace(`https://accounts.spotify.com/authorize?response_type=code&show_dialog=false&redirect_uri=${redirectURI}&scope=${scope}&state=${state}&code_challenge_method=S256&client_id=${clientId}&code_challenge=${codeChallenge}`)
            authCode = this.parseWindow()
            return authCode
        }  
    },
